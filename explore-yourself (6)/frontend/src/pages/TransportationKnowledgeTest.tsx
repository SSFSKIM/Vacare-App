...
import { KnowledgeAnswerGrid } from "../components/KnowledgeAnswerGrid"; // Added import

const STORAGE_KEY = 'transportation_knowledge_test_progress';

export default function TransportationKnowledgeTest() {
  ...
  const { setKnowledgeResults, storeLoading, storeError } = useKnowledgeTestStore('transportation-knowledge');
  ...
  React.useEffect(() => {
    const savedAnswers = localStorage.getItem('transportation-knowledge-answers');
    if (savedAnswers) {
  ...
  React.useEffect(() => {
    localStorage.setItem('transportation-knowledge-answers', JSON.stringify(answers));
  }, [answers]);
  ...
  React.useEffect(() => {
    const fetchQuestions = async () => {
      try {
        ...
        // Filter only transportation questions
        const transportationQuestions = data.filter((q: any) => [
          "Transportation",
          "Aviation",
          "Maritime Transportation"
        ].includes(q.name));
        setQuestions(transportationQuestions);
      } catch (err) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Clear local storage
      localStorage.removeItem('transportation-knowledge-answers');
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto px-4 py-16">
        <div className="max-w-4xl mx-auto">
          <h1 className="text-4xl font-bold mb-8 text-center">Transportation Knowledge Assessment</h1>
          <div className="mb-8">
          ...
          <p className="text-lg text-muted-foreground mb-8 text-center">
            Rate your interest in developing transportation knowledge on a scale from 10 to 100.
