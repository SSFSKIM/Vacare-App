...
export default function TechnicalSkillsTest() {
  ...
  const { setSkillResults, storeLoading, storeError } = useSkillTestStore('technical');
    ...
    const fetchQuestions = async () => {
      try {
        ...
        const filteredQuestions = data.filter((q: any) => 
          technicalSkills.includes(q.name)
        );
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Store the results
      console.log('Setting skill results for technical:', resultsWithSubset);
      await setSkillResults(resultsWithSubset);
      
      navigate("/skill-selection");
    } catch (err) {
      console.error('Error submitting technical skills test:', err);
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto px-4 py-16">
        <div className="max-w-4xl mx-auto">
          <h1 className="text-4xl font-bold mb-8 text-center">Technical Skills Assessment</h1>
          <p className="text-lg text-muted-foreground mb-8 text-center">
            Rate your interest in developing technical skills on a scale from 10 to 100.
            Focus on how passionate you would be to improve in these areas, not your current ability.
          </p>
