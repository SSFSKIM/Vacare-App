...
DEFAULT_MIN_REQUIREMENT_RATIO = 0.60
...
def _calibrate_thresholds(
    ...
    """
    if False:  # DB check removed, using local files
...
def calculate_interest_congruence_all(
    ...
    try:
        interests_df = get_dataframe("elements-interests-csv")
...
async def bootstrap_validation(req: BootstrapValidationRequest) -> BootstrapValidationResponse:
    ...
    try:
        # Load frames
        abil = get_dataframe("elements-abilities-csv")
        skl = get_dataframe("elements-skills-csv")
        knw = get_dataframe("elements-knowledge-2-csv")
        itx = get_dataframe("elements-interests-csv") if req.include_interests else None
