...
export default function EducationTrainingKnowledgeTest() {
  // Initialize Firebase assessment store
  useInitializeFirebaseStore();

  // Get store handlers for knowledge test
  const { setKnowledgeResults, storeLoading: firebaseLoading, storeError: firebaseError } = useKnowledgeTestStore('education-training-knowledge');
  ...
  React.useEffect(() => {
    const savedAnswers = localStorage.getItem('education-training-knowledge-answers');
    if (savedAnswers) {
  ...
  React.useEffect(() => {
    localStorage.setItem('education-training-knowledge-answers', JSON.stringify(answers));
  }, [answers]);
  ...
  React.useEffect(() => {
    const fetchQuestions = async () => {
      try {
        ...
        // Filter only education and training questions
        const educationQuestions = data.filter((q: any) => [
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Clear local storage
      localStorage.removeItem('education-training-knowledge-answers');
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto px-4 py-16">
        <div className="max-w-4xl mx-auto">
          <h1 className="text-4xl font-bold mb-8 text-center">Education and Training Knowledge Assessment</h1>
          <div className="mb-8">
          ...
          <p className="text-lg text-muted-foreground mb-8 text-center">
            Rate your interest in developing education and training knowledge on a scale from 0 to 100.
            Focus on how motivated you would be to learn more in these areas, not your current knowledge level.
          </p>
