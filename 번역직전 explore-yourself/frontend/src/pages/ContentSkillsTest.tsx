...
export default function ContentSkillsTest() {
  ...
  const { setSkillResults, storeLoading, storeError } = useSkillTestStore('content');
  ...
  React.useEffect(() => {
    const fetchQuestions = async () => {
      try {
        ...
        // Filter questions for content skills only
        const contentQuestions = data.filter((q: any) => contentSkills.includes(q.name));
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Store the results
      console.log('Setting skill results for content:', resultsWithSubset);
      await setSkillResults(resultsWithSubset);
      
      navigate("/skill-selection");
    } catch (err) {
      console.error('Error submitting content skills test:', err);
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto px-4 py-16">
        <div className="max-w-4xl mx-auto">
          <h1 className="text-4xl font-bold mb-8 text-center">Content Skills Assessment</h1>
          <p className="text-lg text-muted-foreground mb-8 text-center">
            Rate your interest in developing each content skill on a scale from 10 to 100.
            Focus on how motivated you would be to improve these skills, not your current proficiency.
          </p>
