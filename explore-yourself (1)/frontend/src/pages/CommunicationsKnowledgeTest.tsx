...
const STORAGE_KEY = 'communications_knowledge_test_progress';

export default function CommunicationsKnowledgeTest() {
  ...
  const { setKnowledgeResults, storeLoading, storeError } = useKnowledgeTestStore('communications-knowledge');
  ...
  React.useEffect(() => {
    const savedAnswers = localStorage.getItem('communications-knowledge-answers');
    if (savedAnswers) {
  ...
  React.useEffect(() => {
    localStorage.setItem('communications-knowledge-answers', JSON.stringify(answers));
  }, [answers]);
  ...
  React.useEffect(() => {
    const fetchQuestions = async () => {
      try {
        ...
        // Filter only communications questions
        const communicationsQuestions = data.filter((q: any) => [
          "Telecommunications",
          "Communications and Media"
        ].includes(q.name));
        setQuestions(communicationsQuestions);
      } catch (err) {
  ...
  const handleAnswerChange = (questionId: number, value: number) => { // <-- Changed value type to number
    setAnswers(prev => ({
      ...prev,
      [questionId]: value // <-- No more parseInt needed
    }));
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Clear local storage
      localStorage.removeItem('communications-knowledge-answers');
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto px-4 py-16">
        <div className="max-w-4xl mx-auto">
          <h1 className="text-4xl font-bold mb-8 text-center">Communications Knowledge Assessment</h1>
          <div className="mb-8">
          ...
          <p className="text-lg text-muted-foreground mb-8 text-center">
            Rate your interest in developing communications knowledge on a scale from 0 to 100.
