...
const resourceManagementSkills = [
...
export default function ResourceManagementSkillsTest() {
  ...
  const { setSkillResults, storeLoading, storeError } = useSkillTestStore('resource management');
    ...
    const fetchQuestions = async () => {
      try {
        ...
        const filteredQuestions = data.filter((q: any) => 
          resourceManagementSkills.includes(q.name)
        );
    ...
    try {
      ...
      const resultsWithSubset = {
        ...
        results: responseData.results.map((r: any) => ({
          ...r,
          subset: 'resource management'
        }))
      };
      
      // Store the results
      console.log('Setting skill results for resource management:', resultsWithSubset);
      await setSkillResults(resultsWithSubset);
      
      navigate("/skill-selection");
    } catch (err) {
      console.error('Error submitting resource management skills test:', err);
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto px-4 py-16">
        <div className="max-w-4xl mx-auto">
          <h1 className="text-4xl font-bold mb-8 text-center">Resource Management Skills Assessment</h1>
          <p className="text-lg text-muted-foreground mb-8 text-center">
            Rate your interest in developing resource management skills on a scale from 10 to 100.
            Focus on how eager you would be to cultivate these skills, rather than your current proficiency.
          </p>
